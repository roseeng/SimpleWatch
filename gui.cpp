...
LV_IMG_DECLARE(guitar);
LV_IMG_DECLARE(agno);
LV_IMG_DECLARE(jessica);
LV_IMG_DECLARE(kids);
LV_IMG_DECLARE(prick);
LV_IMG_DECLARE(sailor);
LV_IMG_DECLARE(tree);
LV_IMG_DECLARE(burning);
LV_IMG_DECLARE(teo);
LV_IMG_DECLARE(yoga);
...
int max_images = sizeof(images)/sizeof(void*);
...
/*
    //Create wallpaper
    void *images[] = {
                      //  (void *) &bg, (void *) &bg1, (void *) &bg2, (void *) &bg3,
                          (void *) &guitar,
                          (void *) &agno,
                          (void *) &jessica,
                          (void *) &kids,
...
//    lv_obj_t *img_bin = lv_img_create(scr, NULL);  /*Create an image object*/
    img_bin = lv_img_create(scr, NULL);  /*Create an image object*/
//    srand((int)time(0));
//    int r = rand() % (sizeof(images)/sizeof(void*));

    lv_img_set_src(img_bin, images[current_background]);
...
static void bluetooth_event_cb()
{
    Switch::switch_cfg_t cfg[3] = {{"BT On", bluetooth_sw_event_cb}, {"BT Scan", bluetooth_sw_event_cb}, {"BT Sync", bluetooth_sw_event_cb}};
    sw = new Switch;
    sw->create(cfg, 3, []() {
        delete sw;
        sw = nullptr;
...
static void setting_event_cb()
{
    current_background = (++current_background) % max_images;    
    lv_img_set_src(img_bin, images[current_background]);
